...
void moveNeo(void)
{
	if ((gfTranslateX >= -20.0f) && (gfNeoRotate_Y < 300.0f))
	{
		fprintf(fp, "HERE\n");
		gfTranslateX -= gfSpeed / 30;
	}
	else if (gfNeoRotate_Y <= 300.0f)
		gfNeoRotate_Y += gfSpeed / 30;
//	else 
//	{
//		gfTranslateX += gfSpeed / 30;
//		gfTranslateZ += gfSpeed / 30;
//	}
	updateNormalWalk();
}
...
{
	glPushMatrix();
	glTranslatef(INITIAL_NEO_TRANSLATE_X + gfTranslateX,  INITIAL_NEO_TRANSLATE_Y, INITIAL_NEO_TRANSLATE_Z);
//	glTranslatef(gfTranslateX, INITIAL_NEO_TRANSLATE_Y, gfTranslateZ);

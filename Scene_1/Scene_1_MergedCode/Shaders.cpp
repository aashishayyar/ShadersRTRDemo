...
void moveCamera() {
	...
	if (cameraPosition.y <= watchPosition.y) {
		cameraPosition.y = cameraPosition.y + speed;
	}
	if (centerLookAt.y <= watchPosition.y) {
		centerLookAt.y = centerLookAt.y + speed;
	}
	if (cameraPosition.z >= matrixPosition.z) {
		cameraPosition.z = cameraPosition.z - speed;
	}
}
void toggleFullscreen(void) {
...
void drawMatrixRoom() {
	...
	glPushMatrix();
	glTranslatef(matrixPosition.x, matrixPosition.y, matrixPosition.z);
...
void drawRoom() {
	...
	glVertex3f(-roomSize.xCor, 0.0f, -roomSize.zCor); //Right
	glVertex3f(-roomSize.xCor, roomDepth.yCor, -roomSize.zCor);
	glVertex3f(-roomSize.xCor, roomDepth.yCor, roomSize.zCor);
	glVertex3f(-roomSize.xCor, 0.0f, roomSize.zCor);

	glColor3f(0.2f, 0.2f, 0.2f);
	glVertex3f(roomSize.xCor, 0.0f, -roomSize.zCor); //Left
	glVertex3f(roomSize.xCor, roomDepth.yCor, -roomSize.zCor);
	glVertex3f(roomSize.xCor, roomDepth.yCor, roomSize.zCor);
	glVertex3f(roomSize.xCor, 0.0f, roomSize.zCor);
